...
class PrHandoffTests(unittest.TestCase):
    ...
    def test_written_advisory_rubric_handoff_paths_include_run_dir_separator(self) -> None:
        with tempfile.TemporaryDirectory() as raw:
            run_dir = Path(raw) / ".agent" / "runs" / "run-1"
            ...
            handoff = (run_dir / "pr_handoff.md").read_text()
            self.assertIn("run-1/advisory_review_rubric.md", handoff)
            self.assertIn("run-1/advisory_review_rubric.json", handoff)
            self.assertNotIn("run-1advisory_review_rubric.md", handoff)
